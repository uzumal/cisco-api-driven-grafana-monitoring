...
def store_test_results_in_influxdb(test_results, test_name, is_cpoc):
    ...
    for result in test_results:
        ...
        error_type = result.get('errorType', 'None')  # Get error type, default to 'None' if not present
        print(result)
        if error_type == 'None':
            if is_cpoc:
            ...
            data_point = {
                "measurement": "http_server_test_error",
                "tags": {
                    "test_name": test_name,
                    "agent_name": agent_name
                },
...
def main():
    ...
    for test_id in http_server_test_ids:
        ...
        test_name = http_server_results['test']['testName']
        test_results = http_server_results['results']
        # Process only "Microsoft Office 365 Login" test
